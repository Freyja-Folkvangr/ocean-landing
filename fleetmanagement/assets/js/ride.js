...
(function rideScopeWrapper($) {
    ...
    function updatePoint(location, newContentString) {
        return function () {
            lastDataReceived = location.tst;
            latitude = parseFloat(location.lat)
            longitude = parseFloat(location.lon)
            updatePositionInMap(latitude, longitude, map, newContentString);
        }
    }

    function getVehicleDataWithPlate(locations) {
        $.ajax({
            method: 'POST',
            ...
            success: function receivedData(result) {
                ...
                newContentString =
                    ...
                    '<div id="siteNotice">' +
                    '</div>' +
                    '<h1 id="firstHeading" class="firstHeading">Vehículo:' + vehicle.PlateNumber + '</h1>' +
                    '<div id="bodyContent">' +
                    '<p><b>Estado</b>: ' + ((vehicle.Status == "Active") ? 'Activo' : 'Inactivo') + '</p>' +
                    '<p><b>Empresa</b>: ' + vehicle.Company + '</p>' +
                    '<p><b>Conductor</b>: ' + vehicle.Driver + '</p>' +
                    '</div>' +
                    '</div>';

                locations.forEach(location => {
                    setTimeout(updatePoint(location, newContentString), 500);
                });
    ...
    function getData() {
        $.ajax({
            ...
            success: function receivedData(result) {
                ...
                getVehicleDataWithPlate(locations);
            },
            error: function ajaxError(jqXHR, textStatus, errorThrown) {
                console.error('Error requesting new data: ', textStatus, ', Details: ', errorThrown);
                console.error('Response: ', jqXHR.responseText);
